...
console.log(comicId)
  ...
  return (
    ...
    <div className="row">
        <div className="col-4">
        <img src={`${comic[0].thumbnail.path}/portrait_incredible.${comic[0].thumbnail.extension}`} className="img-fluid" alt="comic_post" />
        ...
        <div className="col-8">
            ...
            <h5>Personajes: </h5>
            <ul>
            {comic[0].characters.items.map((item,idx) => <li key={idx}>{item.name}</li>)}

            </ul>
            <h5>Variantes:</h5>
            <ul>
                {comic[0].variants.map((variant,idx) => <li key={idx}>{variant.name}</li>)}                            
            </ul>
        </div>
    </div>
    </div>
